...
============================================================        =================                ========================
  ...
  |  Mobile App / Client  |
  +-----------+-----------+
              | (1) POST /transfer (with OAuth2 Bearer Token)
              ▼
  +-----------+-----------+
  |   Auckland Ingress    |  -- [JWT Authenticated & Verified]
  |   (Spring Boot App)   |  -- [Executes local account debit]
  +-----------+-----------+
              | (2) JMS Send (Payload: JSON | Header: Auth JWT)
              ▼
    [QM_AKL_FR Gateway] ── (Automatic Workload Balancing)
         /         \
        ▼ (PR1)     ▼ (PR2)
  +-----------+  +-----------+
  |  QM_AKL   |  |  QM_AKL   |  -- (Partial Repositories)
  |   _PR1    |  |   _PR2    |  -- (Active-Active Cluster)
  +-----+-----+  +-----+-----+
        |              |
        +------+-------+ (3) Read JSON message
               ▼
  +-----------+-----------+
  |     IBM ACE v13       |  -- [Decrypts Credentials via mqsivault]
  |   (Active-Active)     |  -- [Cleanses & Transforms JSON to ISO 20022 XML]
  +-----------+-----------+  -- [Preserves JWT Token in MQ RFH2 User Header]
              | (4) Publish Event to Topic: "Payment/CrossBorder/Initiated"
              ▼
...
[SUB_TO_SYDNEY]  [SUB_TO_AUDIT]                                    [SUB_TO_AML]
  (Core Tx)        (Audit Pipeline)                                  (AML Scoring Engine)
      |            |                                                 |
      ▼            ▼                                                 ▼
  [QALIAS]     [QLOCAL]                                          [QLOCAL]
PAYMENT.TO.SYDNEY  PAYMENT.AUDIT.Q                                   PAYMENT.AML.Q
      |            (Dumps payload                                    (Dumps payload 
      ▼            to ELK Index)                                     to AML Analyzer)
[QREMOTE]
PAYMENT.CROSSBORDER.OUT
      |
      ▼ (5) Cluster Routing (PR1/PR2 -> FR Gate -> Sender Channel)
+-----+-----+
|  QM_AKL_FR| -- (Establishes Sender-Receiver Channel over TCP)
+-----+-----+
      |
      |~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ (Over the Tasman Sea)
      |
      ▼ (6) TCP/IP port: 31414
  +-----------+
  |  QM_SYD   | -- (Sydney Gateway Queue Manager)
  +-----+-----+
        |
        ▼ (7) Message Arrives: PAYMENT.CROSSBORDER.OUT (Local Q)
  +-----------+-----------+
  |  Sydney Receiver Serv | -- [Consumes pacs.008 XML via JMS]
  |   (Spring Boot App)   | -- [Extracts JWT from RFH2 & Verifies Offline via Cryptography]
  +-----------+-----------+ -- [Performs Atomic updates to Sydney Balance Database]
              |
              ▼ (8) DB Update (Balance Credit + Audit Trace)
  +-----------+-----------+
  | Sydney PostgreSQL DB  |
  +-----------------------+
```
